...
import numpy as np
from Generate import constants as C
...
def labelToCategorical(labels, sequence_length_first_half, framerate=2, num_classes=3):
	"""
...
def getTimestampTargets(labels):
	...
	targets = np.zeros((labels.shape[0],5,2+labels.shape[-1]), dtype='float')
	...
	for i in np.arange(labels.shape[0]):
		...
		for time_index, class_value in zip(time_indexes, class_values):
			...
			targets[i,counter,1] = time_index/(120*2)
