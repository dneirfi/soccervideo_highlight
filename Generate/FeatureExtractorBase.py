...
class FeatureExtractorBase():
    ...
    def elaborateGame(self, gameDir):
        ...
        npy_FileName =  self.featureName

        logging.info('Searching for data : {0}.npy'.format(npy_FileName))
        #print(gameDir)
        #print(os.listdir(gameDir))
        if (npy_FileName + '.npy' in gameDir) and (self.overwrite == False):
        ...
        #print(os.listdir(gameDir))
            ...
            try:
                features_array = self.elaborateVideo(videoPath=gameDir,
                                                     game_start_time=start_times_of_halves,
                                                     game_end_time=end_times_of_halves)
                #print("HHHHHHHHHHHHHHHHHHH","   ",os.path.join(gameDir, npy_FileName))
                save_dir = 'C:\\Users\\dneir\\PycharmProjects\\CapstoneDesign\\ffff'
                np.save(save_dir, np.array(features_array))
        ...
        logging.debug('Total Elaboration time for the game ' + gameDir + ' : ' + str(
            (time.time() - start_time_elaboration) / 60) + ' min')
        logging.info('Total Elaboration time for the game ' + gameDir + ' : ' + str(
            (time.time() - start_time_elaboration) / 60) + ' min')
        return
    ...
    def readINI(self, gameDir):
        ...
        start_times_of_halves = Start1
        end_times_of_halves = End1
        video_formats = filename1
    ...
    def getVideoFrames(self, videoPath, game_start_index, game_end_index, height, width, handler=''):
        ...
        if (handler == 'OpenCV'):
            ...
            print(game_start_index," ",game_end_index)
            for i in tqdm(range(int(game_start_index), int(game_end_index)), desc='Grabbing Video Frames',
                          unit='frame'):
            ...
            pbar = tqdm(total=game_end_index,
                        ...
                        unit='frame')
            index_frame = 0
            print(index_frame," ",game_start_index," ",game_end_index)
